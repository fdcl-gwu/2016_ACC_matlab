...
% calculate H bound
H = Psi_max/sqrt(2) * trace(G) + 2 * sqrt(Psi_max/b1) * (sin(pi/2 - theta))/(alpha*(beta_max - cos(theta))) ...
    + Psi_max*(beta_max*(beta_max-cos(theta)) + beta_max + cos(theta))/(alpha*(beta_max- cos(theta))^2);
...
% figure out maximum value of c such that all three conditions are
% satisfied
